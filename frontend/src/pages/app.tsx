...
export default function App() {
  ...
  const generateAIImage = async () => {
    ...
    try {
      ...
      if (res.ok && data.url) {
        ...
        toast(data.error || 'Failed to generate image');
      }
    } catch (err) {
      console.error(err);
  ...
  const pushGeneratedImage = async () => {
    if (!aiImageResult || !connectedTV || !selectedLayoutId) return;
    setUploading(true);
    try {
      const newCells = cells.map((c, i) => i === activeCell ? { mediaUrl: aiImageResult, mediaType: 'image' } : c);
      setCells(newCells);
      await updateTV(connectedTV.id, selectedLayoutId, newCells);
      toast(`Live on ${connectedTV.name} — Zone ${activeCell + 1}`);
      setAiImageResult(null);
      setAiPrompt('');
    } catch {
      toast('Failed to push image');
    } finally {
      setUploading(false);
    }
  };
                ...
                <div>
                  ...
                  {uploading ? (
                    ...
                    <>
                      ...
                      <div className="section-label">Generate AI Image for Zone {activeCell + 1}</div>
                      <div style={{ background: '#fff', border: '1px solid #bae6fd', borderRadius: 18, padding: 16, boxShadow: '0 8px 24px rgba(15,23,42,0.03)', marginBottom: 20 }}>
                        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                          ...
                          <button 
                            ...
                            style={{ padding: '10px 16px', background: '#0ea5e9', color: '#fff', border: 'none', borderRadius: 10, cursor: 'pointer', fontWeight: 700, fontSize: 13 }}
                          >
                        ...
                        {generatingImage && (
                          <div style={{ padding: '10px 0', textAlign: 'center', color: '#64748b', fontSize: 12 }}>
                        ...
                        {aiImageResult && !generatingImage && (
                          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, marginTop: 8 }}>
                            <img src={aiImageResult} style={{ width: '100%', borderRadius: 10, border: '1px solid #cbd5e1' }} alt="AI Generated" />
                            <button 
